...
router.route('/')
    .get(verifyToken, async (req, resp) => {
        ...
        const combinedData = 
        usersDBData.map(userDB => {
        const userFile = usersFileData.find(userFile => userFile.id === userDB._doc._id.toString());
        const userPermissions = permissionsData.find(permission => permission.userId === userDB._doc._id.toString());
   ...
   router.route('/:id')
    ...
    .put( verifyToken, (req, resp) => {
        ...
        try {
            ...
            permissionsResult = permissionsFileBL.updatePermission(userData.Id, formattedPermissions);
        } catch (error) {
        ...
        try {
            ...
            const { firstName, lastName, "Session time out": sessionTimeOut, "Created Date": createdDate } = userData;
            const formattedData = {
                id: userData.Id,
                firstName,
                lastName,
        ...
        try {
            ...
            usersDBResult =  userDBBL.updateSingleField(id, 'username', userData["User Name"]);
        } catch (error) {
  ...
  .post(verifyToken, async (req, resp) => {
    ...
    try {
      const { firstName, lastName, sessionTimeOut, createdDate, userName } = userData;
      const formattedData = {
        firstName,
        lastName,
        createdDate,
      ...
      usersDBResult = await userDBBL.createUser({ username: formattedData.username });

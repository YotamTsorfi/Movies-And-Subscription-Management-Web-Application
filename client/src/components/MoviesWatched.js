...
function MoviesWatched({ memberId }) {
  ...
  const fetchSubscriptions = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:4321/subscriptions/${memberId}`,
        {
          //   const response = await axios.get(
          //     `${apiUrlSubs}/subscriptions/${memberId}`,
          //     {
      ...
      console.error(
        `Error fetching subscriptions for member with id ${memberId}`,
        error
      );
      dispatch(logoutUser);
      navigate("/login");
  ...
  const fetchUnwatchedMovies = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4321/subscriptions/unwatched/${memberId}`,
        {
          //   const response = await axios.get(
          //     `${apiUrlSubs}/subscriptions/unwatched/${memberId}`,
          //     {
          headers: { "x-access-token": token },
        }
      );
      ...
      console.error(
        `Error fetching unwatched movies for member with id ${memberId}`,
        error
      );
      dispatch(logoutUser);
      navigate("/login");
  ...
  return (
    ...
    >
      Movies Watched
      <br />
      <button onClick={handleSubscribeButton}>Subscribe to new movie</button>
      {showSubscribe && (
        <div>
      ...
      {memberSubscriptions.map((subscription) =>
        subscription.Movies.map((movie) => {
          if (!movie.movieId) {
            return null; // Skip rendering if movie.movieId is null or undefined
          }
          return (
            <div key={`${subscription._id}-${movie.movieId._id}`}>
              <Link to={`/all-movies/${movie.movieId._id}`}>
                {movie.movieId.Name}
              </Link>

...
function AddUser() {
  ...
  const [user, setUser] = useState({
    ...
    permissions: permissionsList.reduce((acc, permission) => ({ ...acc, [permission]: false }), {}),
  });
  ...
  return (
    <form>
      ...
      <br />
      <TextField label="Username" name="userName" value={user.userName} onChange={handleInputChange} />
      <br />
      <TextField label="Session Time Out" name="sessionTimeOut" value={user.sessionTimeOut} onChange={handleInputChange} />
      <br />
      {permissionsList.map((permission) => (
        <div key={permission}>
            <FormControlLabel        
            control={<Checkbox checked={user.permissions[permission]} onChange={handleCheckboxChange} name={permission} />}
            label={permission}
            />
        </div>
        ))}
